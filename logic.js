...
// If one of these variables value became true , it means the player won.
...
function setGame() {
  ...
  //Goal, we will use the backend board to design and move the tile of the frontend board

  // Loops are code to repeat tasks inside it, until it fulfill its task
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      //This code is to create a tile through creating divs elements
...
function handleSlide(e) {
  console.log(e.code); //represents what key is being pressed
...
function slideLeft() {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < columns; c++) {
      ...
      //update color using updateTIle function
      updateTile(tile, num);
...
function slideUp() {
  for (let c = 0; c < columns; c++) {
    ...
    // Loop to update thew id of the tile and its color
    for (let r = 0; r < rows; r++) {
      board[r][c] = row[r];
